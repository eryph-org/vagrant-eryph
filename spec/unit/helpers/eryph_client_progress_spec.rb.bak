...
RSpec.describe VagrantPlugins::Eryph::Helpers::EryphClient, 'log message display' do
  ...
  describe '#wait_for_operation log message handling' do
    ...
    it 'displays log messages like .NET client verbose output' do
      ...
      # Should show log messages with operation ID prefix
      log_messages = ui_messages.select { |msg| msg.start_with?("Operation #{operation_id}:") }

      expect(log_messages).to include("Operation #{operation_id}: Pulling volume gene:dbosoft/winsrv2019-standard/20250911:sda (hyperv/amd64) (234 MiB / 4604 MiB) => 5,1 % completed")
      expect(log_messages).to include("Operation #{operation_id}: Pulling volume gene:dbosoft/winsrv2019-standard/20250911:sda (hyperv/amd64) (464 MiB / 4604 MiB) => 10,1 % completed")
      expect(log_messages).to include("Operation #{operation_id}: Configure Catlet CPU count: 2")
    end
